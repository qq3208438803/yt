...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
	...
	else
	{
		...
		snprintf(queryStr, sizeof(queryStr),
				...
				#ifdef ENABLE_KILL_STATISTICS
				",statistics_jinno_kills, statistics_shinsoo_kills, statistics_chunjo_kills, statistics_total_kills,statistics_total_deaths,"
				"statistics_duels_won, statistics_duels_lost, statistics_bosses_kills, statistics_stones_kills "
				#endif
#ifdef RETURN_BACK_SYSTEM
				",dungeonindex "
#endif
...
bool CreatePlayerTableFromRes(MYSQL_RES * res, TPlayerTable * pkTab)
{
	...
	#ifdef ENABLE_EXTEND_INVEN_SYSTEM1
	str_to_number(pkTab->extend_inven, row[col++]);
#endif
#ifdef ENABLE_GAYA_SYSTEM
