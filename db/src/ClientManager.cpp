...
{
#ifdef ENABLE_OPEN	
	CDBManager::instance().DirectQuery("UPDATE player2.player SET combat_zone_rank = 0 WHERE combat_zone_rank > 0");

	std::auto_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery("SELECT * FROM player2.combat_zone_ranking_weekly ORDER BY memberPoints DESC LIMIT 3"));	
	...
	if (pMsg->Get()->uiNumRows == 3)
	{
		...
		{
			#ifdef ENABLE_OPEN	
			sprintf(szQuery, "UPDATE player2.player SET combat_zone_rank = '%d' WHERE name = '%s'", memberRank, row[0]);
			#else
...
#ifdef ENABLE_OPEN	
	CDBManager::instance().DirectQuery("TRUNCATE TABLE player2.combat_zone_ranking_weekly");
	#else
...
#ifdef ENABLE_OPEN		
	sprintf(szQuery, 
		"INSERT INTO player2.combat_zone_skills_cache (pid, skillLevel1, skillLevel2, skillLevel3, skillLevel4, skillLevel5, skillLevel6) "
